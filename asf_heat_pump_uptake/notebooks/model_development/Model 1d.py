...
tidy_uptake_loans = (
    pandas.concat(
        [
            uptake_loans[f"year_{year}"].value_counts().rename(f"{year}")
            for year in range(2023, 2051)
        ],
        axis=1,
    )
...
ax.plot(
    ...
    color="dodgerblue",
    label="Heat Pump, No Loans",
)

ax.plot(
    tidy_uptake_loans.loc[lambda df: df["index"] == "Gas Boiler", "year"],
    tidy_uptake_loans.loc[lambda df: df["index"] == "Gas Boiler", "prop"],
    color="r",
    label="Gas Boiler, Loans",
)

ax.plot(
    tidy_uptake_loans.loc[lambda df: df["index"] == "Heat Pump", "year"],
    tidy_uptake_loans.loc[lambda df: df["index"] == "Heat Pump", "prop"],
    color="indianred",
    label="Heat Pump, Loans",
)
